...
class AddEditForm extends React.Component {
  ...
  submitFormAdd = async (e) => {
    ...
    if (!this.state.password)return NotificationManager.error("Please Enter Password", "Info", 2000);
    ...
    if (this.state.valid === true) {
      ...
      if (rs) {
        this.props.addItemToState(rs != true);
  ...
  if (rs) {
    this.props.updateState(rs != true);
  ...
  validation = (e) => {
    ...
    // let firmNumber = this.state.firm_phone
    if (!this.state.firm_name)return NotificationManager.error("Please Enter Firm Name", "Info", 2000);
    else if(CONFIG.FIRM_EMAIL)  if (!this.state.firm_email)return NotificationManager.error("Please Enter Firm Email", "Info", 2000);
    // else if(CONFIG.FIRM_PHONE)  if (!firmNumber )return NotificationManager.error("Please Enter Firm Phone No", "Info", 2000);
    // else if(CONFIG.FIRM_PHONE)  if (!firmNumber === 0 )return NotificationManager.error("Please Enter Firm Phone No", "Info", 2000);
    // else if(CONFIG.FIRM_PHONE)  if (firmNumber.length > 12 && firmNumber.length < 9 && firmNumber.length === 11) return NotificationManager.error("Invalid Phone No", "Info", 2000);
    // else if(CONFIG.FIRM_ADDRESS)  if (!this.state.firm_address)return NotificationManager.error("Please Enter Firm Address", "Info", 2000);
    // else if(CONFIG.FIRM_DISTRICT)  if (!this.state.firm_district)return NotificationManager.error("Please Enter Firm District", "Info", 2000);
    // else if(CONFIG.FIRM_STATE)  if (!this.state.firm_state)return NotificationManager.error("Please Enter Firm State", "Info", 2000);
    // else if(!this.state.selectedDivision)return NotificationManager.error("Please Select Divisions", "Info", 2000);
    // else if(CONFIG.FIRM_GST_NUMBER) if (!this.state.gst_number)return NotificationManager.error("Please Enter GST No", "Info", 2000);
    // else if(CONFIG.FIRM_DRUG_LICENCE)  if (!this.state.drug_license)return NotificationManager.error("Please Enter Drug License", "Info", 2000);
    // else if(CONFIG.FIRM_BANK_NAME)  if (!this.state.bank_name)return NotificationManager.error("Please Enter Bank Name", "Info", 2000);
    // else if(CONFIG.FIRM_BANK_ACC_NO)  if (!this.state.bank_acc_no)return NotificationManager.error("Please Enter Bank Account No", "Info", 2000);
    // else if(CONFIG.FIRM_BANK_IFSC)  if (!this.state.bank_ifsc)return NotificationManager.error("Please Enter IFSC Code", "Info", 2000);
    // else if(CONFIG.FIRM_BANK_PAYEE_NAME)  if (!this.state.bank_payee_name)return NotificationManager.error("Please Enter Payee Name", "Info", 2000);
    
    else if (!this.state.name)return NotificationManager.error("Please Enter Distributor Name", "Info", 2000);
    else if (!this.state.email)return NotificationManager.error("Please Enter Distributor email", "Info", 2000);
    else if (!this.state.phone)return NotificationManager.error("Please Enter Distributor Phone", "Info", 2000);
    else if (this.state.phone.length > 12 && this.state.phone.length < 9 && this.state.phone.length === 11)
    return NotificationManager.error("Invalid Phone No", "Info", 2000);
    // else if(CONFIG.REP_DOB) if (!this.state.dob)return NotificationManager.error("Please Enter Distributor DOB", "Info", 2000);
    // else if(CONFIG.REP_OP_AREA) if (!this.state.op_area)return NotificationManager.error("Please Enter Operation Area", "Info", 2000);
    // else if(CONFIG.REP_AADHAR_NO) if (!this.state.aadhar_no)return NotificationManager.error("Please Enter Aadhar No", "Info", 2000);
    else {
  ...
  async componentDidMount() {
    if (this.props.item) {
      const { id, name, email, phone, address,  op_area, aadhar_no, password } = this.props.item;
      this.setState({id, name, email, phone, address, op_area, aadhar_no, password});
      ...
      let fcRs = await GetFranchisee({
        id : fId })
      if (fcRs){
        this.setState({frId : fcRs.id})
        this.setState({firm_name : fcRs.name})
        this.setState({firm_email : fcRs.email})
        this.setState({firm_phone : fcRs.phone})
        this.setState({firm_address : fcRs.address  })
        this.setState({firm_state : fcRs.state})
        this.setState({firm_district : fcRs.district})
        this.setState({gst_number : fcRs.gst_number})
        this.setState({drug_license : fcRs.drug_license})
        this.setState({bank_name : fcRs.bank_name})
        this.setState({bank_acc_no : fcRs.bank_acc_no})
        this.setState({bank_ifsc : fcRs.bank_ifsc})
        this.setState({bank_payee_name : fcRs.bank_payee_name })
        let divList = []
        if (Array.isArray(fcRs.divisions)){
          fcRs.divisions.map((it) => {
    ...
    if (rs){
      this.setState({divisionsList : rs})
    }
  }
          ...
          <div className="col-md-6">
            ...
            {CONFIG.FIRM_PHONE && (
              <div className="form-group row">
                ...
                <div className="col-sm-10">
                  <input type="number" value={this.state.firm_phone} name="firm_phone"
                    onChange={this.onChange} className="form-control" />
                </div>
              </div>
            )}
            ...
            {CONFIG.FIRM_GST_NUMBER && (
              <div className="form-group row">
                ...
                <div className="col-sm-10">
                  <input type="tel" value={this.state.gst_number} name="gst_number"
                    onChange={this.onChange} className="form-control" />
                </div>
              </div>
            )}
            ...
            {CONFIG.FIRM_DRUG_LICENCE && (
              <div className="form-group row">
                ...
                <div className="col-sm-10">
                  <input type="tel" value={this.state.drug_license} name="drug_license"
                    onChange={this.onChange} className="form-control" />
                </div>
              </div>
            )}
